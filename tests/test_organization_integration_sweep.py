...
class TestLambdaAppDeployInitStackCallSite(unittest.TestCase):
    ...
    if-not-ok control flow exactly."""
...
class TestLambdaAppRaiseIfSubmitFailed(unittest.TestCase):
    ...
    and return a SUBMIT_FAILED record instead of raising (so the CLI's
    sweep can report it). The Lambda entrypoint has no sweep - it signals
    failure purely by raising - so _raise_if_submit_failed must restore
    that pre-existing contract, or a real deployment failure would be
    silently swallowed and the Lambda would report success anyway."""
    ...
    def test_none_and_empty_are_noop(self):
        self.app._raise_if_submit_failed(None)
        self.app._raise_if_submit_failed([])

    def test_normal_record_does_not_raise(self):
        self.app._raise_if_submit_failed({
            "account": "1", "name": "n", "region": "us-east-1",
            "stack_type": "response", "stack_name": "s", "stack_id": "sid"})
        self.app._raise_if_submit_failed([{
            "account": "1", "name": "n", "region": "us-east-1",
            "stack_type": "collection", "stack_name": "s", "stack_id": "sid"}])

    def test_single_submit_failed_record_raises(self):
        # deploy_response_stack returns a single dict, not a list.
        with self.assertRaises(Exception):
            self.app._raise_if_submit_failed({
                "account": "1", "name": "n", "region": "us-east-1",
                "stack_type": "response", "stack_name": "s", "stack_id": None,
                "final_status": "SUBMIT_FAILED", "status_reason": "LimitExceededException"})

    def test_submit_failed_within_a_list_raises(self):
        # deploy_all_collection_stacks/deploy_eks_audit_logs_stacks return
        # lists - one failed region among several successful ones must still
        # be caught, not averaged away.
        with self.assertRaises(Exception):
            self.app._raise_if_submit_failed([
                {"account": "1", "name": "n", "region": "us-east-1",
                 "stack_type": "collection", "stack_name": "s1", "stack_id": "sid1"},
                {"account": "1", "name": "n", "region": "eu-west-1",
                 "stack_type": "collection", "stack_name": "s2", "stack_id": None,
                 "final_status": "SUBMIT_FAILED", "status_reason": "ThrottlingException"},
            ])

    def test_wired_into_response_stack_call_site_in_ready_branch(self):
        # Not just a dead helper - confirm it's actually reached from a real
        # call site (the READY-account branch's response-stack deploy).
        app = self.app
        sub_account = ("123456789012", "acct-name")
        ...
        graph_client.get_accounts.return_value = [{
            ...
            "display_name": "acct-name", "cloud_regions": ["us-east-1"],
        ...
        with patch.object(app, "boto3") as boto3_mock, \
                patch.object(app, "deploy_response_stack", return_value={
                    "account": "123456789012", "name": "acct-name", "region": "us-east-1",
                    "stack_type": "response", "stack_name": "s", "stack_id": None,
                    "final_status": "SUBMIT_FAILED", "status_reason": "boom"}):
            boto3_mock.Session.return_value = session
            with self.assertRaises(Exception):
                app.integrate_sub_account(
                    sub_account, sts_client, graph_client, ["us-east-1"], 42,
                    None, None, "OrganizationAccountAccessRole", org_account_id,
                    environment="env", domain="streamsec.io", response=True)

    def test_wired_into_eks_audit_logs_call_site_in_ready_branch(self):
        # deploy_eks_audit_logs_stacks returns a LIST (unlike
        # deploy_response_stack's single dict) - a genuinely different shape
        # through _raise_if_submit_failed, worth its own wiring test.
        app = self.app
        sub_account = ("123456789012", "acct-name")
        org_account_id = "123456789012"
        sts_client = MagicMock()
        ...
        with patch.object(app, "boto3") as boto3_mock, \
                patch.object(app, "deploy_eks_audit_logs_stacks", return_value=[
                    {"account": "123456789012", "name": "acct-name", "region": "us-east-1",
                     "stack_type": "eks_audit", "stack_name": "s1", "stack_id": "sid1"},
                    {"account": "123456789012", "name": "acct-name", "region": "eu-west-1",
                     "stack_type": "eks_audit", "stack_name": "s2", "stack_id": None,
                     "final_status": "SUBMIT_FAILED", "status_reason": "boom"},
                ]):
            boto3_mock.Session.return_value = session
            with self.assertRaises(Exception):
                app.integrate_sub_account(
                    sub_account, sts_client, graph_client, ["us-east-1"], 42,
                    None, None, "OrganizationAccountAccessRole", org_account_id,
                    environment="env", domain="streamsec.io", eks_audit_logs=True)

    def test_wired_into_collection_stacks_call_site_in_ready_branch(self):
        app = self.app
        sub_account = ("123456789012", "acct-name")
        org_account_id = "123456789012"
        sts_client = MagicMock()
        ...
        graph_client.get_accounts.return_value = [{
            "cloud_account_id": sub_account[0], "status": "READY",
            "display_name": "acct-name", "cloud_regions": ["us-east-1"],
            # No realtime regions yet -> regions_to_integrate is non-empty ->
            # reaches the deploy_all_collection_stacks call site.
            "realtime_regions": []}]
        graph_client.get_account_response_config.return_value = {"remediation": {"status": "ok"}}
        ...
        with patch.object(app, "boto3") as boto3_mock, \
                patch.object(app, "deploy_all_collection_stacks", return_value=[
                    {"account": "123456789012", "name": "acct-name", "region": "us-east-1",
                     "stack_type": "collection", "stack_name": None, "stack_id": None,
                     "final_status": "SUBMIT_FAILED", "status_reason": "boom"},
                ]):
            boto3_mock.Session.return_value = session
            with self.assertRaises(Exception):
